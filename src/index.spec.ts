...
describe("Toy Robot Simulator", () => {
  describe("Robot", () => {
    ...
    it(`should execute "Place 2,2,North"`, () => {
      ...
      expect(myRobot.direction).toBe(Direction.North);
      expect(myRobot.position).toEqual({ x: 2, y: 2 });
    });
    ...
    it(`should execute one place after the other`, () => {
      ...
      expect(myRobot.direction).toBe(Direction.East);
      expect(myRobot.position).toEqual({ x: 1, y: 1 });

      myRobot.runCmd("Place 2,1,South");
      expect(myRobot.direction).toBe(Direction.South);
      expect(myRobot.position).toEqual({ x: 2, y: 1 });
    });
    ...
    it(`should move towards South when there is space`, () => {
      ...
      myRobot.runCmd("Move");
      expect(myRobot.direction).toBe(Direction.South);
      expect(myRobot.position).toEqual({ x: 0, y: 4 });
    });
    it(`shouldn't move towards South when there is no space`, () => {
      myRobot.runCmd("Place 0,0,South");
      myRobot.runCmd("Move");
      expect(myRobot.direction).toBe(Direction.South);
      expect(myRobot.position).toEqual({ x: 0, y: 0 });
    });

    it(`should move towards West when there is space`, () => {
      myRobot.runCmd("Place 5,0,West");
      myRobot.runCmd("Move");
      expect(myRobot.direction).toBe(Direction.West);
      expect(myRobot.position).toEqual({ x: 4, y: 0 });
    });
